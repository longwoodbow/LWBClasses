...
void GiveSpawnResources(CRules@ this, CBlob@ blob, CPlayer@ player, WarPlayerInfo@ w_info)
{
	...
	{
		GiveFactoryItem(blob, "mat_poisonmeats");
	}
	else if (blob.getName() == "demolitionist")
	{
		GiveFactoryItem(blob, "mat_bombboxes");
	...
	{
		// keep order reversed for proper picking default ammo
		if (!GiveFactoryItem(blob, "mat_boomerangs"))
		{
			// make arrows out of air
			if (w_info.canGetWeaponthrowerItems)
			{
				MakeMaterial(blob, "mat_boomerangs", 15);
				w_info.canGetMusketmanItems = false;
			}
		}
		GiveFactoryItem(blob, "mat_chakrams");
	}
	else if (blob.getName() == "firelancer")
	{
		// keep order reversed for proper picking default ammo
		if (!GiveFactoryItem(blob, "mat_firelances"))
		{
			// make arrows out of air
		...
		GiveFactoryItem(blob, "mat_flamethrowers");
	}
	else if (blob.getName() == "gunner")
	{
		// keep order reversed for proper picking default ammo
		if (!GiveFactoryItem(blob, "mat_bullets"))
		{
	...
	if (victim !is null)
	{
		...
		if (core !is null)
		{
			...
			if (w_info !is null)
			{
				...
				w_info.canGetWeaponthrowerItems = true;
				w_info.canGetFirelancerItems = true;
