...
void onTick(CBrain@ this)
{
	...
	if (target !is null)
	{
		...
		const bool gotweapons = hasWeapons(blob);
		if (!gotweapons)
		{
		...
		if (visibleTarget)
		{
			...
			if ((!blob.isKeyPressed(key_action1) && getGameTime() % 300 < 240 && distance < 30.0f + 3.0f * difficulty) || !gotweapons)
...
void AttackBlob(CBlob@ blob, CBlob @target)
{
	...
	if (targetDistance > 15.0f)
	{
		...
		if (!fireTime && (fTime == 0 || XORRandom(130 - 5.0f * difficulty) == 0))		// difficulty
		{
			...
			const u32 shootTime = Maths::Max(WeaponthrowerVars::throw_charge, Maths::Min(uint(targetDistance * (0.3f * Maths::Max(130.0f, vert_dist) / 100.0f) + XORRandom(20)), WeaponthrowerVars::throw_charge_level2));
