...
void LoadSprites(CSprite@ this)
{
	...
	case PLAYER_ARMOUR_CAPE:
		ensureCorrectRunnerTexture(this, "duelist_cape", "DuelistCape");
		break;
	case PLAYER_ARMOUR_GOLD:
		ensureCorrectRunnerTexture(this, "duelist_gold",  "DuelistGold");
		break;
	}
...
void onTick(CSprite@ this)
{
	...
	DuelistInfo@ duelist;
	if (!blob.get("duelistInfo", @duelist))
	{
		return;
	}

	doRopeUpdate(this, blob, duelist);
	...
	else if (duelist.state == DuelistStates::resheathing_slash)
	{
		this.SetAnimation("resheath_slash");
	}
	else if(duelist.state == DuelistStates::resheathing_cut)
	{
		this.SetAnimation("draw_rapier");
	}
	else if (duelist.state == DuelistStates::rapier_drawn)
	{
		if (duelist.rapierTimer < DuelistVars::slash_charge)
		{
			this.SetAnimation("draw_rapier");
		}
		else if (duelist.rapierTimer < DuelistVars::slash_charge_limit)
		{
	...
	else if (isRapierAnim(this) && !this.isAnimationEnded())// now playing rapier? keep going
	{
		...
		if(duelist.state == DuelistStates::rapier_power)
		{
			u8 mintime = 6;
			u8 maxtime = 8;
			if (duelist.rapierTimer >= mintime && duelist.rapierTimer <= maxtime)
			{
				wantsChopLayer = true;
				chopframe = duelist.rapierTimer - mintime;
				chopAngle = -vec.Angle();
			}
		}
	}
	else if (duelist.state == DuelistStates::rapier_cut || duelist.state == DuelistStates::rapier_power)
	{
...
void doRopeUpdate(CSprite@ this, CBlob@ blob, DuelistInfo@ duelist)
{
	CSpriteLayer@ rope = this.getSpriteLayer("rope");
	CSpriteLayer@ hook = this.getSpriteLayer("hook");

	bool visible = duelist !is null && duelist.grappling;
	...
	Vec2f adjusted_pos = Vec2f(duelist.grapple_pos.x, Maths::Max(0.0, duelist.grapple_pos.y));
	...
	hook.ResetTransform();
	if (duelist.grapple_id == 0xffff) //still in air
	{
		duelist.cache_angle = -duelist.grapple_vel.Angle();
	}
	hook.RotateBy(duelist.cache_angle , Vec2f());

	hook.TranslateBy(off);
	hook.SetIgnoreParentFacing(true);
	hook.SetFacingLeft(false);

	//GUI::DrawLine(blob.getPosition(), duelist.grapple_pos, SColor(255,255,255,255));
}
