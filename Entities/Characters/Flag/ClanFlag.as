...
void LoadSprites(CSprite@ this)
{
	...
	if(player !is null)
	{
		...
		if(clantag == "[JP]" || clantag == "JP") blob.Tag("JapanesePlayer");
		else
		{
			string playerName = player.getUsername();
			if(playerName == "kinokino7[Zipangu]" || playerName == "taroimop") blob.Tag("JapanesePlayer");
			else return;
		}
	}
	else return;
	...
	CSpriteLayer@ flag = this.addSpriteLayer("flag", "JapaneseFlag.png" , 32, 32);
...
void onTick(CSprite@ this)
{
	...
	CBlob@ blob = this.getBlob();
	if(!(blob.hasTag("JapanesePlayer"))) return;
	...
	if (quiverLayer !is null)
	{
		...
		bool down = (
			((blob.getName() == "archer" ||blob.getName() == "musketman" || blob.getName() == "assassin" || blob.getName() == "duelist" || blob.getName() == "firelancer" || blob.getName() == "gunner") && this.isAnimation("crouch")) || 
...
void onGib(CSprite@ this)
{
	...
	CParticle@ Body     = makeGibParticle("JapaneseFlag.png", pos, vel + getRandomVelocity(90, hp , 80), 0, 0, Vec2f(32, 32), 2.0f, 20, "/BodyGibFall", team);
}
