...
void onInit(CInventory@ this)
{
	...
	if (!blob.exists("blocks"))
	{
		...
		{
			BuildBlock b(0, "climbablebarricade", "$climbablebarricade$", "Climbable Barricade");
...
void onCreateInventoryMenu(CInventory@ this, CBlob@ forBlob, CGridMenu@ gridmenu)
{
	...
	AddIconToken("$HandcannonBarricade$", "Entities/Characters/Handcannon/HandcannonIcons.png", Vec2f(16, 32), 2, blob.getTeamNum());
	...
	if (menu !is null)
	{
		...
		{
			CGridButton @button = menu.AddButton("$HandcannonNothing$", "Stop Building", blob.getCommandID("stop_building"));
			if (button !is null)
			{
				button.selectOneOnClick = true;
				if (handcannon.buildmode == HandcannonBuilding::nothing)
				{
		...
		{
			CGridButton @button = menu.AddButton("$HandcannonBarricade$", "Build Barricade", blob.getCommandID("barricade"));
			if (button !is null)
			{
				button.selectOneOnClick = true;
				if (handcannon.buildmode == HandcannonBuilding::barricade)
				{
					button.SetSelected(1);
				}
			}
...
void onCycle(CBitStream@ params)
{
	...
	u8 type = handcannon.buildmode;

	type++;
	if (type >= HandcannonBuilding::count)
	{
		type = 0;
	}

	CycleToActionType(this, handcannon, type);
}
...
void onCommand(CInventory@ this, u8 cmd, CBitStream@ params)
{
	...
	if (cmd == blob.getCommandID("stop_building") && isServer())
	{
		...
		handcannon.buildmode = HandcannonBuilding::nothing;
	}
	else if (cmd == blob.getCommandID("barricade") && isServer())
	{
		processingTempBlob(blob, @blocks[0], 0);
		handcannon.buildmode = HandcannonBuilding::barricade;
	}
...
void CycleToActionType(CBlob@ this, HandcannonInfo@ handcannon, u8 actionType)
{
	handcannon.buildmode = actionType;
	if (this.isMyPlayer())
	{
	...
	switch(actionType)
	{
		case HandcannonBuilding::nothing:
			commandname = "stop_building";
		break;

		case HandcannonBuilding::barricade:
