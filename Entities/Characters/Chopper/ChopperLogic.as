...
class AxeDrawnState : ChopperState
{
	...
	bool TickState(CBlob@ this, ChopperInfo@ chopper, RunnerMoveVars@ moveVars)
	{
		...
		if (getGameTime() > 150)
		{
			SetHelp(this, "help self action", "chopper", getTranslatedString("$Help_Axe$ Slash!    $KEY_HOLD$$RMB$"), "", 255);
		}
...
class SlashState : ChopperState
{
	...
	bool TickState(CBlob@ this, ChopperInfo@ chopper, RunnerMoveVars@ moveVars)
	{
		...
		else if (delta > DELTA_BEGIN_ATTACK && delta < 10)
		{
			...
			DoAttack(this, chopper.state == ChopperStates::chop_power ? 2.0f : 1.0f, -(vec.Angle()), 120.0f, Hitters::axe, delta, chopper);
		}
...
void onTick(CBlob@ this)
{
	...
	if (ismyplayer)
	{
		...
		if (u_showtutorial && !this.hasTag("spoke description"))
		{
			...
			this.Chat("I'm a warrior who specialize in wood!\n\n[LMB] to build\n[RMB] to melee(select tool in inventory)");
...
void HandlePickaxeCommand(CBlob@ this, CBitStream@ params)
{
	...
	else
	{
		...
		if (b !is null)
		{
			...
			if (isServer())
			{
				this.server_Hit(b, tilepos, attackVel, attack_power, Hitters::mattock, teamHurt);
