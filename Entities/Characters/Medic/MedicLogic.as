...
void onInit(CBlob@ this)
{
	...
	SetHelp(this, "help self action2", "medic", getTranslatedString("$Grapple$ Grappling hook    $RMB$"), "", 255);
...
void onTick(CBlob@ this)
{
	...
	if (responsible)
	{
		...
		if (u_showtutorial && !this.hasTag("spoke description"))
		{
			...
			this.Chat("Heal allies and spray liquid!\n\n[LMB] to heal ally\n[RMB] to grapple\n[SPACE] to use splay(buy jar at builder shop)\nEveryone can use med kit on press [V], but uneffective for other classes");
...
void onAddToInventory(CBlob@ this, CBlob@ blob)
{
	...
	if (this.isMyPlayer())
	{
		if (itemname == "mat_medkits")
		{
			SetHelp(this, "help self action", "medic", "$mat_medkits$ Heal        $LMB$", "", 255);
			SetHelp(this, "help self action", "medic", "Press V key to Heal yourself using med kits", "", 255);
		}
		else
		{
			for (uint j = 0; j < sprayTypeNames.length; j++)
			{
				if (itemname == sprayTypeNames[j])
				{
					SetHelp(this, "help inventory", "medic", "$Help_Spray$Spray         $KEY_SPACE$", "", 255);
