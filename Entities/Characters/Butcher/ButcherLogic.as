...
void onInit(CBlob@ this)
{
	...
	SetHelp(this, "help self action", "butcher", getTranslatedString("$KitchenKnife$Butcher corpse$LMB$"), "", 255);
...
{
	if (customData == Hitters::kitchenknife)
	{
		if (damage > 0.0f && hitBlob.hasTag("flesh") && hitBlob.hasTag("dead") && !hitBlob.hasTag("butched") && hitBlob.getHealth() - hitBlob.get_f32("gib health") <= 0.0f)
		{
			...
			s16 poisonTimer = hitBlob.exists(poison_timer) ? hitBlob.get_s16(poison_timer) : 0;
			...
			if (getNet().isServer())
			{
				...
				if (meat !is null)
				{
					meat.Tag('custom quantity');
			 		...
			 		meat.server_SetQuantity(poisoned ? 5 : 3);
			 	}
				if(!poisoned)
				{
...
void onAddToInventory(CBlob@ this, CBlob@ blob)
{
	if (blob.getName() == "mat_poisonmeats")
		SetHelp(this, "help self action2", "butcher", "$mat_poisonmeats$ Throw poisonous meat $RMB$", "", 255);
	if (blob.getName() == "mat_cookingoils")
		SetHelp(this, "help inventory", "butcher", "$mat_cookingoils$Burn/Cook steak or fish $KEY_SPACE$", "", 255);
}