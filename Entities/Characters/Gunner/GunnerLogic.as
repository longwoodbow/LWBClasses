...
void onInit(CBlob@ this)
{
	...
	SetHelp(this, "help self hide", "gunner", getTranslatedString("Hide    $KEY_S$"), "", 255);
	SetHelp(this, "help self action2", "gunner", getTranslatedString("$Grapple$ Grappling hook    $RMB$"), "", 255);

	AddIconToken("$Help_Gunner1$", "LWBHelpIcons.png", Vec2f(8, 16), 26);
	AddIconToken("$Help_Gunner2$", "LWBHelpIcons.png", Vec2f(8, 16), 27);
	SetHelp(this, "help inventory", "gunner", "$Help_Gunner1$$Swap$$Help_Gunner2$$KEY_TAP$$KEY_F$Change Full charge shoot", "", 255);
...
void onTick(CBlob@ this)
{
	...
	if (u_showtutorial && this.isMyPlayer() && !this.hasTag("spoke description"))
	{
		...
		this.Chat("Light guns and grapple!\n\n[LMB] to shoot bullet, snipe or double shoot on full charge(select in inventory)\n[RMB] to grapple");
...
void onAddToInventory(CBlob@ this, CBlob@ blob)
{
	...
	if (this.isMyPlayer())
	{
		...
		{
			SetHelp(this, "help self action", "gunner", getTranslatedString("$Help_Bullet$Fire bullet   $KEY_HOLD$$LMB$"), "", 255);
		}
	}
}
