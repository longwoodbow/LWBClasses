...
void onInit(CBlob@ this)
{
	...
	SetHelp(this, "help self action2", "crossbowman", getTranslatedString("$Help_Bayonet$Bayonet/Fletch    $RMB$"), "", 255);
...
void onTick(CBlob@ this)
{
	...
	if (u_showtutorial && myplayer && !this.hasTag("spoke description"))
	{
		...
		this.Chat("Triple shoot with mechanical bow!\n\n[LMB] to shoot arrow, triple shoot on full charge\n[RMB] to use bayonet, can make arrows from wooden thing, also pickup arrow");
		...
		if (swordState)
		{
			...
			else
			{
				...
				else if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
				{
					...
					DoAttack(this, 1.0f, attackAngle, attackarc, Hitters::bayonet, delta, crossbowman);
				}
				else if (delta >= 18)
				{
					state = CrossbowmanVars::not_aiming;
...
void onAddToInventory(CBlob@ this, CBlob@ blob)
{
	...
	if (this.isMyPlayer())
	{
		for (uint j = 0; j < arrowTypeNames.length; j++)
		{
			...
			{
				SetHelp(this, "help self action", "crossbowman", getTranslatedString("$arrow$Fire arrow   $KEY_HOLD$$LMB$"), "", 255);
