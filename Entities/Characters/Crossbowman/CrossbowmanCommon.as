...
bool ReceiveArrowState(CBlob@ this, CBitStream@ params)
{
	...
	CrossbowmanInfo@ crossbowman;
	if (!this.get("crossbowmanInfo", @crossbowman)) { return false; }

	crossbowman.arrow_type = 0;
	if (!params.saferead_u8(crossbowman.arrow_type)) { return false; }

	if (isServer())
	{
		CBitStream reserialized;
		reserialized.write_u8(crossbowman.arrow_type);
