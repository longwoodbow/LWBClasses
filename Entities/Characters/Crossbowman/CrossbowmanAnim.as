...
void onTick(CSprite@ this)
{
	...
	bool legolas = crossbowman.state == CrossbowmanVars::legolas_ready || crossbowman.state == CrossbowmanVars::legolas_charging;
	needs_shiny = false;
	...
	else if (firing || legolas)
	{
		...
		{
			this.SetAnimation(isCrouching(blob) ? "shoot_crouched" : "shoot");
		}
	}
	else if (crossbowman.state == CrossbowmanVars::sword_cut_mid)
	{
		this.SetAnimation("strike_mid");
		if (crossbowman.swordTimer <= 1)
			this.animation.SetFrameIndex(0);
	}
	else if (crossbowman.state == CrossbowmanVars::sword_cut_mid_down)
	{
		this.SetAnimation("strike_mid_down");
		if (crossbowman.swordTimer <= 1)
			this.animation.SetFrameIndex(0);
	}
	else if (crossbowman.state == CrossbowmanVars::sword_cut_up)
	{
		this.SetAnimation("strike_up");
		if (crossbowman.swordTimer <= 1)
			this.animation.SetFrameIndex(0);
	}
	else if (crossbowman.state == CrossbowmanVars::sword_cut_down)
	{
...
void DrawBow(CSprite@ this, CBlob@ blob, CrossbowmanInfo@ crossbowman, f32 armangle, const u8 arrowType, Vec2f armOffset)
{
	...
	if (!crossbowman.has_arrow || crossbowman.state == CrossbowmanVars::no_arrows || crossbowman.state == CrossbowmanVars::legolas_charging)
	{
		string animname = "default";
		
		if (crossbowman.state != CrossbowmanVars::legolas_charging)
		{
			u16 frontframe = 0;
			f32 temp = Maths::Min(crossbowman.charge_time, CrossbowmanVars::ready_time);
	...
	else if (crossbowman.state == CrossbowmanVars::readying)
	{
		u16 frontframe = 0;
	...
	else if (crossbowman.state != CrossbowmanVars::fired || crossbowman.state == CrossbowmanVars::legolas_ready)
	{
		u16 frontframe = Maths::Min((crossbowman.charge_time / (CrossbowmanVars::shoot_period_1 + 1)), 2);
		setArmValues(frontarm, true, armangle, 0.1f, "default", Vec2f(-4.0f * sign, 0.0f), armOffset);
		frontarm.animation.frame = frontframe;
		...
		if (crossbowman.state == CrossbowmanVars::legolas_ready)
		{
			needs_shiny = true;
