...
void onInit(CBlob@ this)
{
	...
	SetHelp(this, "help self action", "spearman", getTranslatedString("$Help_SpearJab$Jab        $LMB$"), "", 255);
	SetHelp(this, "help self action2", "spearman", getTranslatedString("$Help_SpearThrow$Throw    $RMB$"), "", 255);
...
void onTick(CBlob@ this)
{
	...
	*/
	...
	if (myplayer)
	{
		...
		{
			SetHelp(this, "help self action", "spearman", getTranslatedString("$Help_SpearPower$ Slash!    $KEY_HOLD$$LMB$"), "", 255);
		}
		else if (this.isKeyJustPressed(key_action2) && getGameTime() > 150)
		{
			SetHelp(this, "help self action", "spearman", getTranslatedString("$Help_SpearThrow$ Throw!    $KEY_HOLD$$RMB$"), "", 255);
		}
	}
...
class SlashState : SpearmanState
{
	...
	bool TickState(CBlob@ this, SpearmanInfo@ spearman, RunnerMoveVars@ moveVars)
	{
		...
		else if (delta > DELTA_BEGIN_ATTACK && delta < 10)
		{
			...
			DoAttack(this, 2.0f, -(vec.Angle()), 60.0f, Hitters::spear, delta, spearman);//half arc
		}
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, SpearmanInfo@ info)
{
				...
				for (int j = 0; j < rayInfos.size(); j++)
				{
					...
					if (rayb.getTeamNum() != this.getTeamNum() || rayb.hasTag("dead player"))
					{
						...
						if ((type == Hitters::fire && rayb.hasScript("IsFlammable.as")) ||
							(type == Hitters::poisoning && rayb.hasScript("IsPoisonable.as")) &&
						...
						this.server_Hit(rayb, rayInfos[j].hitpos, velocity, temp_damage, Hitters::spear, true);// normal hit
					}
	...
	if(!(type == Hitters::spear) && usedSpecialSpear && this.get_u8("specialhit") == 0)// cost special spear
	{
