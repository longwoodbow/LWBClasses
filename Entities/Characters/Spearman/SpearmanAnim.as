...
void onTick(CSprite@ this)
{
	...
	if (blob.hasTag("dead"))
	{
		...
		doEffectUpdate(this, blob, false, spearman, false);
	...
	else if(spearman.state == SpearmanStates::resheathing_throw)
	{
		this.SetAnimation("resheath_throw");
	}
	else if(spearman.state == SpearmanStates::resheathing_cut || spearman.state == SpearmanStates::resheathing_slash)
	{
			this.SetAnimation(crouching ? "draw_spear_crouched" : "draw_spear");
	}
	else if (spearman.state == SpearmanStates::spear_drawn)
	{
		if (spearman.spearTimer < SpearmanVars::slash_charge)
		{
			this.SetAnimation(crouching ? "draw_spear_crouched" : "draw_spear");
		}
		else if (spearman.spearTimer < SpearmanVars::slash_charge_level2)
		{
			this.SetAnimation(spearman.throwing ? (crouching ? "throw_ready_crouched" : "throw_ready") : (crouching ? "strike_power_ready_crouched" : "strike_power_ready"));
			this.animation.frame = 0;
		}
		else if (spearman.spearTimer < SpearmanVars::slash_charge_limit)
		{
			this.SetAnimation(spearman.throwing ? (crouching ? "throw_ready_crouched" : "throw_ready") : (crouching ? "strike_power_ready_crouched" : "strike_power_ready"));
	...
	else if (spearman.state == SpearmanStates::spear_cut_mid_down)
	{
		this.SetAnimation("strike_mid_down");
	}
	else if (spearman.state == SpearmanStates::spear_cut_up)
	{
		this.SetAnimation("strike_up");
	}
	else if (spearman.state == SpearmanStates::spear_cut_down)
	{
		this.SetAnimation("strike_down");
	}
	else if (spearman.state == SpearmanStates::spear_power || spearman.state == SpearmanStates::spear_power_super)
	{
		...
		if (spearman.spearTimer <= 1)
			this.animation.SetFrameIndex(0);
		if (spearman.spearTimer < 3)
			this.animation.timer = 0;

		u8 mintime = 6;
		u8 maxtime = 8;
		if (spearman.spearTimer >= mintime && spearman.spearTimer <= maxtime)
		{
			wantsChopLayer = true;
			chopframe = spearman.spearTimer - mintime;
			chopAngle = -vec.Angle();
		}
	}
	else if (spearman.state == SpearmanStates::spear_throw || spearman.state == SpearmanStates::spear_throw_super)
	{
	...
	doEffectUpdate(this, blob, hasAnySpears(blob), spearman, shinydot);
...
void doEffectUpdate(CSprite@ this, CBlob@ blob, bool has_spears, SpearmanInfo@ spearman, bool shinydot)
{
	...
	if (quiverLayer !is null)
	{
		...
		if(spearman.spear_type == SpearType::fire && hasSpears(blob, SpearType::fire) && getGameTime() % 6 == 0)
			makeFireParticle(effectPos, 4);
		if(spearman.spear_type == SpearType::poison && hasSpears(blob, SpearType::poison) && getGameTime() % 12 == 0)
