...
void onTick(CSprite@ this)
{
	...
	doRopeUpdate(this, blob, assassin);
	...
	else if (action1)
	{
		...
		if (direction == -1)
		{
			this.SetAnimation(assassin.use_left ? "stab_up_left" : "stab_up");
		}
		else if (direction == 0)
		{
			this.SetAnimation(assassin.use_left ? "stab_mid_left" : "stab_mid");
		}
		else
		{
			this.SetAnimation(assassin.use_left ? "stab_down_left" : "stab_down");
		}
		assassin.use_left = assassin.use_left == false; //true > false > true...
	}
	else if (inair)
	{
...
void doRopeUpdate(CSprite@ this, CBlob@ blob, AssassinInfo@ assassin)
{
	CSpriteLayer@ rope = this.getSpriteLayer("rope");
	CSpriteLayer@ hook = this.getSpriteLayer("hook");

	bool visible = assassin !is null && assassin.grappling;
	...
	Vec2f adjusted_pos = Vec2f(assassin.grapple_pos.x, Maths::Max(0.0, assassin.grapple_pos.y));
	...
	hook.ResetTransform();
	if (assassin.grapple_id == 0xffff) //still in air
	{
		assassin.cache_angle = -assassin.grapple_vel.Angle();
	}
	hook.RotateBy(assassin.cache_angle , Vec2f());

	hook.TranslateBy(off);
	hook.SetIgnoreParentFacing(true);
	hook.SetFacingLeft(false);

	//GUI::DrawLine(blob.getPosition(), assassin.grapple_pos, SColor(255,255,255,255));
}
