...
void onInit(CBlob@ this)
{
	...
	AddIconToken("$Help_WarHammerJab$", "LWBHelpIcons.png", Vec2f(16, 16), 22);
	AddIconToken("$Help_WarHammerSlash$", "LWBHelpIcons.png", Vec2f(16, 16), 23);
	AddIconToken("$Help_Flail$", "LWBHelpIcons.png", Vec2f(16, 16), 21);

	SetHelp(this, "help self action", "warhammer", getTranslatedString("$Help_WarHammerJab$Jab        $LMB$"), "", 255);
	SetHelp(this, "help self action2", "warhammer", "$Help_Flail$Flail    $RMB$", "", 255);
...
void onTick(CBlob@ this)
{
	...
	if (u_showtutorial && myplayer && !this.hasTag("spoke description"))
	{
		...
		this.Chat("Slow but powerful weapons!\n\n[LMB] to jab/slash\n[RMB] to spin flail, release to throw, can glide in air while spinning flail");
	...
	*/
	...
	if (myplayer)
	{
		...
		{
			SetHelp(this, "help self action", "warhammer", getTranslatedString("$Help_WarHammerSlash$ Slash!    $KEY_HOLD$$LMB$"), "", 255);
		}
		if (this.isKeyJustPressed(key_action2) && getGameTime() > 150)
		{
			SetHelp(this, "help self action", "warhammer", "$Help_Flail$ Throw!    $KEY_HOLD$$RMB$", "", 255);
		}
	}
...
class SlashState : WarhammerState
{
	...
	bool TickState(CBlob@ this, WarhammerInfo@ warhammer, RunnerMoveVars@ moveVars)
	{
		...
		else if (delta > DELTA_BEGIN_ATTACK && delta < 13)
		{
			...
			DoAttack(this, 3.0f, -(vec.Angle()), 120.0f, Hitters::warhammer, delta, warhammer);
		}
