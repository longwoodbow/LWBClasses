...
void onInit(CBlob@ this)
{
	...
	AddIconToken("$Help_Stick$", "LWBHelpIcons.png", Vec2f(16, 16), 19);
	AddIconToken("$Help_Lance1$", "LWBHelpIcons.png", Vec2f(8, 16), 36);
	AddIconToken("$Help_Lance2$", "LWBHelpIcons.png", Vec2f(8, 16), 37);

	SetHelp(this, "help self hide", "firelancer", getTranslatedString("Hide    $KEY_S$"), "", 255);
	SetHelp(this, "help self action2", "firelancer", getTranslatedString("$Help_Stick$ Stick Attack $RMB$"), "", 255);
...
void onTick(CBlob@ this)
{
	...
	if (u_showtutorial && this.isMyPlayer() && !this.hasTag("spoke description"))
	{
		...
		this.Chat("Chinese medieval boom stick!\n\n[LMB] to ignite firelance and shoot frags\n[RMB] to melee with stick");
		...
		for (int i = hitInfos.size() - 1; i >= 0; i--)
		{
			...
			if (b !is null)
			{
				...
				for (int j = 0; j < rayInfos.size(); j++)
				{
					...
					{
						this.server_Hit(rayb, rayInfos[j].hitpos, velocity, temp_damage, Hitters::stick, true);
					}
...
void onAddToInventory(CBlob@ this, CBlob@ blob)
{
	...
	if (this.isMyPlayer())
	{
		for (uint j = 0; j < lanceTypeNames.length; j++)
		{
			if (itemname == lanceTypeNames[j])
			{
				SetHelp(this, "help self action", "firelancer", getTranslatedString("$mat_firelances$Fire lance   $KEY_HOLD$$LMB$"), "", 255);
				if (j > 0 && this.getInventory().getItemsCount() > 1)
				{
					SetHelp(this, "help inventory", "firelancer", "$Help_Lance1$$Swap$$Help_Lance2$         $KEY_TAP$$KEY_F$", "", 2);
				}
				break;
			}
		}
