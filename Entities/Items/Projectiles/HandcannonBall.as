...
const f32 ballMediumSpeed = 8.0f;
const f32 ballFastSpeed = 13.0f;

const f32 BALL_PUSH_FORCE = 6.0f;
const f32 SPECIAL_HIT_SCALE = 1.0f; //special hit on food items to shoot to team-mates

const s32 FIRE_IGNITE_TIME = 5;

const u32 STUCK_BALL_DECAY_SECS = 30;
...
void onInit(CBlob@ this)
{
	...
	this.set_u8("custom_hitter", Hitters::handcannon);
}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		if (dmg > 0.0f)
		{
			...
			this.server_Hit(blob, point1, initVelocity, dmg, Hitters::handcannon);
...
bool doesCollideWithBlob(CBlob@ this, CBlob@ blob)
{
	...
	//collide so normal balls can be ignited
	...
	bool check =	this.getTeamNum() != blob.getTeamNum() || // collide with enemy blobs
					blob.getName() == "bridge" ||
					(blob.getName() == "keg" && !blob.isAttached() && this.hasTag("fire source")); // fire balls collide with team kegs that arent held
...
bool specialBallHit(CBlob@ blob)
{
...
void Pierce(CBlob @this, CBlob@ blob = null)
{
	...
	{
		BallHitMap(this, end, this.getOldVelocity(), 0.5f, Hitters::handcannon, blob);
	}
}

f32 BallHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	if (hitBlob !is null)
	{
		Pierce(this, hitBlob);
		...
		if (specialBallHit(hitBlob))
		{
			const f32 scale = SPECIAL_HIT_SCALE;
			f32 force = (BALL_PUSH_FORCE * 0.125f) * Maths::Sqrt(hitBlob.getMass() + 1) * scale;
			//if (this.hasTag("bow ball"))
			{
				force *= 1.3f;
			}
		...
		{
			BallHitMap(this, worldPoint, velocity, damage, Hitters::handcannon, hitBlob);
		}
		//die otherwise
		else
		{
...
void BallHitMap(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, u8 customData, CBlob@ hitBlob = null)
{
	if (velocity.Length() > ballFastSpeed)
	{
		this.getSprite().PlaySound("BallHitGroundFast.ogg");
	}
	else
	{
		this.getSprite().PlaySound("BallHitGround.ogg");
	}
	...
	// saving information on what was hit to determine when the ball should collapse
	if (isServer())
	{
	...
	//ignite fireplace when fire ball hits the base
	CBlob@[] blobsInRadius;
	if (getMap().getBlobsInRadius(worldPoint, this.getRadius() * 1.3f, @blobsInRadius))
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			{
				this.server_Hit(b, worldPoint, Vec2f(0, 0), velocity.Length() / 7.0f, Hitters::handcannon);
				break;
			}
		}
	}
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	customData == Hitters::spear ||
	customData == Hitters::bayonet ||
	customData == Hitters::stab ||
	customData == Hitters::shovel ||
	customData == Hitters::axe ||
	customData == Hitters::warhammer ||
	customData == Hitters::flail ||
	customData == Hitters::rapier ||
	customData == Hitters::kitchenknife ||
	customData == Hitters::stick ||
	customData == Hitters::pike_thrust || 
	customData == Hitters::pike_slash ||
	customData == Hitters::handaxe) // changed
	{
		return 0.0f; //no cut balls
	}

	return damage;
}

f32 getBallDamage(CBlob@ this, f32 vellen = -1.0f)
{
	...
	if (vellen >= ballFastSpeed)
	{
		return 1.0f;
	}
	else if (vellen >= ballMediumSpeed)
	{
		return 1.0f;
	}
