...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		if (dmg > 0.0f)
		{
			...
			// or give poison
			else if (spearType == SpearType::poison && !this.hasTag("no_poison"))
...
void Pierce(CBlob @this, CBlob@ blob = null)
{
	...
	{
		SpearHitMap(this, end, this.getOldVelocity(), 0.5f, Hitters::thrownspear, blob);
	}
}
...
f32 SpearHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData, const u8 spearType)
{
	if (hitBlob !is null)
	{
		...
		else if(spearType == SpearType::poison && hitShield)
		{
			// as same as fire
			this.Tag("no_poison");
			this.server_Die();
		...
		{
			SpearHitMap(this, worldPoint, velocity, damage, Hitters::thrownspear, hitBlob);
		}
		//die otherwise
		else
		{
