...
const f32 boomerangMediumSpeed = 8.0f;
const f32 boomerangFastSpeed = 13.0f;

const f32 BOOMERANG_PUSH_FORCE = 6.0f;
//arrow is 6.0f
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		{
			dmg = getBoomerangDamage(this, vellen);
		}

		// this isnt synced cause we want instant collision for arrow even if it was wrong
		dmg = BoomerangHitBlob(this, point1, initVelocity, dmg, blob, Hitters::boomerang);
		...
		if (dmg > 0.0f)
		{
			...
			this.server_Hit(blob, point1, initVelocity, dmg, Hitters::boomerang);
		}
	}
}
...
bool specialBoomerangHit(CBlob@ blob)
{
...
void Pierce(CBlob @this, CBlob@ blob = null)
{
	...
	{
		BoomerangHitMap(this, end, this.getOldVelocity(), 0.5f, Hitters::boomerang, blob);
	}
}

f32 BoomerangHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	if (hitBlob !is null)
	{
		Pierce(this, hitBlob);
		...
		if (specialBoomerangHit(hitBlob))
		{
			const f32 scale = SPECIAL_HIT_SCALE;
			f32 force = (BOOMERANG_PUSH_FORCE * 0.125f) * Maths::Sqrt(hitBlob.getMass() + 1) * scale;
		...
		if (!hitShield)
		{
			if (velocity.Length() > boomerangFastSpeed)
			{
...
void BoomerangHitMap(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, u8 customData, CBlob@ hitBlob = null)
{
	if (velocity.Length() > boomerangFastSpeed)
	{
		this.getSprite().PlaySound("hit_wood.ogg");
	...
	if (getMap().getBlobsInRadius(worldPoint, this.getRadius() * 1.3f, @blobsInRadius))
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			{
				this.server_Hit(b, worldPoint, Vec2f(0, 0), velocity.Length() / 7.0f, Hitters::boomerang);
				break;
			}
		}
	}
...
void onThisAddToInventory(CBlob@ this, CBlob@ inventoryBlob)
{
	...
	for (int i = 0; i < inventoryBlob.getInventory().getItemsCount(); i++)
	{
		...
		if (blob !is this && blob.getName() == "mat_boomerangs")
		{
...
void onHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	...
	if (this !is hitBlob && customData == Hitters::boomerang)
	{
		// affect players velocity
		const f32 scale = specialBoomerangHit(hitBlob) ? SPECIAL_HIT_SCALE : 1.0f;

		Vec2f vel = velocity;
		const f32 speed = vel.Normalize();
		if (speed > 17.59f * 0.5f)
		{
			f32 force = (BOOMERANG_PUSH_FORCE * 0.125f) * Maths::Sqrt(hitBlob.getMass() + 1) * scale * 1.3f;// like bow arrow
...
f32 getBoomerangDamage(CBlob@ this, f32 vellen = -1.0f)
{
	...
	if (vellen >= boomerangFastSpeed)
	{
		return 1.0f;
	}
	else if (vellen >= boomerangMediumSpeed)
	{
		return 0.75f;
	}
