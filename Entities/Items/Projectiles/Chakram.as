...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		dmg = ChakramHitBlob(this, point1, initVelocity, dmg, blob, Hitters::chakram);

		if (dmg > 0.0f)
		{
			//perform the hit and tag so that another doesn't happen
			this.server_Hit(blob, point1, initVelocity, dmg, Hitters::chakram);
			this.Tag("collided");
		}
	}
}
...
void Pierce(CBlob @this, CBlob@ blob = null)
{
	...
	{
		ChakramHitMap(this, end, this.getOldVelocity(), 0.5f, Hitters::chakram, blob);
	}
}
...
f32 ChakramHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	if (hitBlob !is null)
	{
		...
		{
			ChakramHitMap(this, worldPoint, velocity, damage, Hitters::chakram, hitBlob);
		}
		//die otherwise
		else
		{
...
void ChakramHitMap(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, u8 customData, CBlob@ hitBlob = null)
{
	...
	if (map.getBlobsInRadius(worldPoint, this.getRadius() * 1.3f, @blobsInRadius))
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			{
				this.server_Hit(b, worldPoint, Vec2f(0, 0), velocity.Length() / 7.0f, Hitters::chakram);
				break;
			}
		}
	}
...
void onHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	...
	if (this !is hitBlob && customData == Hitters::chakram)
	{
